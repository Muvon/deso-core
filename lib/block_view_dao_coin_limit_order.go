...
func (bav *UtxoView) _connectDAOCoinLimitOrder(
	...
	// the disconnect case.
	matchingOrders, err := bav.GetNextLimitOrdersToFill(txn, transactorOrder, nil, blockHeight)
	if err != nil {
	...
	for len(matchingOrders) > 0 {
		...
		matchingOrders, err = bav.GetNextLimitOrdersToFill(txn, transactorOrder, lastSeenOrder, blockHeight)
		if err != nil {
			return 0, 0, nil, errors.Wrapf(err,
	...
	createUTXO := func(amountNanos uint64, publicKeyArg []byte, utxoType UtxoType) (utxo *UtxoEntry, _err error) {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "_connectDAOCoinLimitOrder: Problem adding output utxo")
		}
		...
		for _, daoCoinPKIDIter := range sortedInnerMapPKIDs {
			...
			if daoCoinPKID == ZeroPKID {
				...
				if newDESOSurplus.Uint64() != 0 {
					if utxo, err = createUTXO(newDESOSurplus.Uint64(), pubKey, UtxoTypeDAOCoinLimitOrderPayout); err != nil {
						return 0, 0, nil, err
					}
...
func (bav *UtxoView) GetNextLimitOrdersToFill(txn *MsgDeSoTxn,
	...
	glog.Infof("TX: %s LAST: %v  NEXT: %v ", txn.Hash().String(), lastSeenOrder, matchingOrders)
